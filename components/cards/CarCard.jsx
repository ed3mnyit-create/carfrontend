...
const CarCard = ({ car, variant = "light" }) => {
  ...
  const cardClasses = isDark
    ? "bg-white/5 border border-white/10 hover:border-primary/30 shadow-lg hover:shadow-primary/10"
    : "bg-white rounded-[2rem] border border-slate-200 hover:border-primary/30 shadow-md hover:shadow-[0_25px_50px_-12px_rgba(10, 35, 115,0.25)]";
  ...
  const priceContainerClasses = isDark
    ...
    : "bg-primary-light border border-primary/15";
  ...
  const specItemClasses = isDark
    ...
    : "bg-slate-50/80 border border-slate-100 hover:border-primary/20 hover:bg-white";

  const specIconClasses = isDark ? "text-primary" : "text-primary";
  ...
  return (
    ...
    >
      {!isDark && (
        <div className="absolute inset-0 bg-gradient-to-br from-primary-light/50 via-white to-primary-light/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0" />
      )}
      ...
      <div className={`relative ${imageContainerClasses} w-full shrink-0 overflow-hidden z-10 ${isDark ? 'rounded-t-2xl' : ''}`}>
        ...
        />
        <div className={`absolute inset-0 bg-gradient-to-t ${isDark ? 'from-black/80 via-transparent to-transparent opacity-70' : 'from-slate-900/80 via-transparent to-transparent opacity-60'}`} />
        ...
        {car.region && (
          <div className="absolute bottom-3 left-3 flex flex-col gap-2">
            <span className={`${isDark ? 'bg-primary/90 backdrop-blur-md border-primary/30' : 'bg-primary/90 backdrop-blur-md border-primary/30'} text-white text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1.5 shadow-lg backdrop-blur-sm`}>
      ...
      <div className={`${isDark ? 'p-5 sm:p-6' : 'p-3 sm:px-5 sm:pb-5 sm:pt-3'} flex flex-col grow relative z-10`}>
        ...
        <div className="mt-auto flex gap-4">
          ...
          <Link
            ...
            className="flex-[1.2] bg-gradient-to-l from-primary to-primary-hover text-white text-center py-3 sm:py-3.5 rounded-xl font-black text-xs sm:text-sm hover:from-primary-hover hover:to-primary-hover transition-all shadow-[0_10px_20px_-10px_rgba(10, 35, 115,0.5)] hover:shadow-[0_15px_25px_-10px_rgba(10, 35, 115,0.6)] active:scale-95 flex items-center justify-center gap-2 relative overflow-hidden group/btn"
          >
